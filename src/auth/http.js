...
api.interceptors.response.use(
  ...
  async (err) => {
    ...
    // treat any auth endpoint as "do not refresh"
    const isAuthCall =
      url.includes("/login") ||
      url.includes("/signup") ||
      url.includes("/refresh") ||
    ...
    // - it is 401
    // - it's NOT an auth call
    // - we haven't retried yet
    if (status !== 401 || isAuthCall || original._retry) {
      // Don't mask server payload; just propagate the error
    ...
    try {
      ...
      // update session blob if provided
      ...
      // propagate the ORIGINAL error if we were handling login 401s,
      // but since we're here only for non-auth 401s, pass refresh error
