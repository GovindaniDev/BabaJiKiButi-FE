...
export default function CartPage({ userId }) {
  ...
  const totals = useMemo(() => {
    ...
    for (const it of items) {
      ...
      const effectiveMrp = mrp > 0 ? mrp : sell; // if no mrp, treat selling as mrp to avoid negative discount
    ...
    <div className="max-w-[1120px] mx-auto my-4 grid grid-cols-1 lg:grid-cols-[1fr_340px] gap-4 px-3">
      ...
      <div>
        ...
        <div className="bg-white border border-[#f1d6cf] rounded-xl shadow-sm p-3 flex flex-wrap gap-3">
          <form onSubmit={checkPin} className="flex items-center gap-2 border border-dashed border-[#f1d6cf] rounded-lg px-3 py-2">
            <input
              value={pin}
      ...
      {/* RIGHT */}
    {
      isEmpty ? null : (
        ...
        <div className="bg-white border border-[#f1d6cf] rounded-xl shadow-sm p-4 lg:sticky lg:top-[72px]">
          ...
          <button onClick={handlePlaceOrder} className="w-full mt-3 h-11 text-[15px] rounded-lg font-semibold bg-[#f7c9b8] text-[#3a2a28]">
            PLACE ORDER
          </button>
