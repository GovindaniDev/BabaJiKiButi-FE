...
/* =========================================================
   UTILS
========================================================= */
const STEP_MS = 5000; // slide every 5s
const SLIDE_DUR = 0.8;
...
function Card({ p, isMember }) {
  ...
  const canShowMembersPrice =
    !!membersText && !!membersNum && (!priceNum || membersNum < priceNum);
  const showMembersRow = isMember && p.badge !== "New" && canShowMembersPrice;
  const showNotMemberRow = !showMembersRow;
  ...
  return (
    <div className="pc-card shrink-0 px-3 w-1/2 md:w-1/3 lg:w-1/4">
      <div className="pc-cardBox group h-full flex flex-col overflow-hidden rounded-xl bg-white shadow-md transition hover:-translate-y-0.5 hover:shadow-xl">
        ...
        >
          <img src={src} alt={p.title} className="h-70 w-full object-contain" />
          <div className="absolute right-2 top-2 z-10 flex items-center gap-2">
            {p.badge && (
              <span
                className={`rounded-full px-3 py-1 text-[12px] font-extrabold shadow ${BADGE_CLASS(
                  p.badge
                )}`}
              >
                {p.badge}
        ...
        <div className="pc-details flex flex-1 flex-col gap-2 p-4">
          ...
          <div className="mt-1 flex flex-wrap items-baseline gap-2">
            {priceText && (
              <div className="text-[20px] font-extrabold text-gray-900">{priceText}</div>
            )}
...
function ProductCards({ title, products, isMember = true }) {
  ...
  // clone first N for seamless loop
  const items = useMemo(() => {
    ...
    <section ref={rootRef} className="pc-section mx-auto w-full rounded-2xl bg-[#faedae] px-4 py-8">
      <header ref={headerRef} className="pc-header mb-3">
        ...
        <p className="mt-1 text-sm text-gray-500">
          All attributes (taglines, badges, MRP & sale price) are dynamic & admin-driven.
        </p>
      </header>
      ...
      <div className="relative w-full h-150 overflow-hidden">
        ...
        <div ref={trackRef} className="flex flex-nowrap will-change-transform">
          {items.map((p, i) => (
            <Card key={`${p.id ?? p.slug}-${i}`} p={p} isMember={true} />
          ))}
        </div>
      </div>
    </section>
...
export default function NutritionPage() {
  ...
  const [cards, setCards] = useState(PRODUCTS_DEFAULT);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        setLoading(true);
        const list = await getAllProducts(); // from /api/products/all
        if (!mounted) return;

        // Map backend ProductDto -> card shape the slider expects
        const mapped =
          Array.isArray(list) && list.length
            ? list.map((p) => {
                ...
                return {
                  ...
                  price: price ? `₹${price}` : "",
                  mrp: mrp ? `₹${mrp}` : "",
                  membersPrice: "", // if you add membership pricing later
                  badge: (p.tags && Array.from(p.tags)[0]) || null, // first enum badge
                  discount,
        ...
        setCards(PRODUCTS_DEFAULT); // graceful fallback
      } finally {
        setLoading(false);
      }
    })();
